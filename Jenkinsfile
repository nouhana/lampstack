...
node {
	...
	sh "docker tag dockerstack_php_apache:B${BUILD_NUMBER} registry12/dockerstack_php_apache:B${BUILD_NUMBER}"
	sh "docker tag dockerstack_db:db-B${BUILD_NUMBER} registry12/dockerstack_db:db-B${BUILD_NUMBER}"
	sh "docker push registry12/dockerstack_php_apache:B${BUILD_NUMBER}"
	sh "docker push registry12/dockerstack_db:db-B${BUILD_NUMBER}"
